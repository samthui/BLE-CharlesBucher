...
{
    //save
    BLEDiscoveryHelper* discoveryCentral = [BLEDiscoveryHelper sharedInstance];
    Device* owner = (Device*)[discoveryCentral.discoveredDeviceList objectAtIndex:_ownerIndex];
    CheckTableItem checkItem = [Utilities findSavedObject:[Utilities UUIDofPeripheral:owner.peripheral]];
    if (checkItem.found) {
        NSMutableArray* storedDeviceList = [Utilities arrayFromUserDefaultWithKey:(NSString*)kStoredDeviceList];
        DeviceConfig* deviceConfig = (DeviceConfig*)[storedDeviceList objectAtIndex:checkItem.index];
        ...
        //
        int dataToSend = bit0*pow(2, 0) + bit1*pow(2, 1) + bit2*pow(2, 2) + bit3*pow(2, 3);
        
        deviceConfig.state = dataToSend;
        ...
        [Utilities saveToUserDefaultWithKey:(NSString*)kStoredDeviceList forArray:storedDeviceList];
    }
}
...
//    NSString* distanceText = [Utilities describeDistanceFromRange:[Utilities convertToRangeDistanceFromRSSI:rssi]];
 //   NSString* distanceText = [Utilities describeDistanceFromRSSI:rssi];
    NSString* distanceText = [NSString stringWithFormat:@"%f", [Utilities convertToDistanceFromRSSI:rssi]];
